...
| 咖啡店候选 | 6 | 比较办公、社区、文化休闲、综合消费、同业饱和和外围成长画像 |
| 便利店候选 | 6 | 比较社区、办公、学校、换乘、同业饱和和车辆服务画像 |
| POI | 1,157 | 验证不同半径、类别、数量、密度和距离指标 |
| POI 类别 | 30 | 覆盖商业、居住、公共服务、交通、物流、竞品和互补业态 |
| 候选空间图层 | 4 | 每类项目各一层，包含不同面积的候选多边形 |
| 约束空间图层 | 4 | 每类项目各一层，用于相交和最近距离验证 |
...
3. Fixture Adapter 按类别和半径过滤 1,157 条记录；选择 `auto / amap / overpass` 时，Provider Adapter 自动分页或构造受控在线查询。
...
Workbench 和 DOCX 报告都会展示这些字段。测试还要求 24 个候选全部有场景 POI、四类项目分别产生至少 4 种不同软评分，避免只是复制同一份数据换名称。
...
因此当前正确表述是：“系统已能在 24 个差异化合成候选和 1,157 条可复现 POI 上完成多场景候选证据比较。”不应表述为：“系统已经掌握真实城市 POI、真实客流，能够给出可靠商业选址结论。”
