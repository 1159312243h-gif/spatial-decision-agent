...
- 完成 Hot100 第 1 题“两数之和”的暴力法
- 完成第 1 题的哈希表法
- 理解“先检查补数，再保存当前数字”可以避免重复使用同一元素
- 完成 Hot100 第 49 题“字母异位词分组”
- 理解排序后的字符串可以作为异位词的统一哈希键
- 理解 `defaultdict(list)` 会为不存在的键自动创建空列表

### 复杂度

- 两数之和暴力法：时间 `O(n²)`，额外空间 `O(1)`
- 两数之和哈希表法：平均时间 `O(n)`，空间 `O(n)`
- 字母异位词排序法：时间 `O(n × k log k)`，空间 `O(n × k)`